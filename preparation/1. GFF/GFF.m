function [F, weight] = GFF(I,r1,eps1,r2,eps2)
...
% Laplician Filtering to get the HF component 
H = LapFilter(G);
% Gaussian Filtering to get the saliency maps 
S = GauSaliency(H);
...
function [ H ] = LapFilter( I )
%Conduct Laplacian filtering on each source image
L = [0 1 0; 1 -4 1; 0 1 0]; % The 3*3 laplacian filter
N = size(I,3);
I = double(I)/255;
H = zeros(size(I,1),size(I,2),N); % Assign memory
for i = 1:N
    H(:,:,i) = abs(imfilter(I(:,:,i),L,'replicate'));
end
